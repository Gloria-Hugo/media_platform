  ...
  <div class="margin-box">
    ...
    <div class="form-box">
      ...
      >
        ...
        <el-table-column prop="channel_money" label="结算金额"></el-table-column>
        <el-table-column prop="demand_state" label="状态">
          <template slot-scope="scope">
            ...
            >
            <el-button
              v-else-if="scope.row.channel_state == 3"
              type="text"
              size="small"
              style="color: #949aa6"
              >已结算</el-button
  ...
  methods: {
    async setMoney() {
      ...
      const { data: res } = await this.$http.get(
        `/statisticsMobileAction/settlementDemand?type=2&channel_by=${token}`
      );
      if (res.netCode == 200) {
    ...
    async getSetllementDemandList(status, page) {
      ...
      let token = sessionStorage.getItem("userName");
      const { data: res } = await this.$http.get(
        `/demandMobileAction/getSettlementDemandList?type=4&user_code=${token}&state=${status}&offset=${page}&limit=${this.pageSize}`
      );
      if (res.netCode==200) {
    ...
    exportExcel() {
      /* out-table关联导出的dom节点  */
         ...
         try {
             FileSaver.saveAs(new Blob([wbout], { type: 'application/octet-stream' }), '结算数据.xlsx')
